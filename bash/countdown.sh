...
#CTRL \ triggers this function to apply on running script
function foundsecret {
  stty sane
  clear
  dialog --msgbox "you found the secret of getting out of this script." 7 60
  exit
}
#Ctrl C triggers this function to reset timer and continue \
dialog not working properly with this type of scenario \
so making it running by running another dialog with requirements
function reset {
  clear
  echo ""
  sleepCount=$numberOfSleeps
  doCountdown |dialog --gauge "You are not allowed to get out of this script." 7 60
}
